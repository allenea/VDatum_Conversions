...
def read_vdatum_regions():
    ...
    new:
    https://vdatum.noaa.gov/download/data/vdatum_regional_20230907.zip
    (should be this but not working...)

    The files from Sept 2023 are in here for now:
        https://vdatum.noaa.gov/download/data/vdatum_all_20230907.zip

    You have to do that step manually. I did a command line diff check to 
    see what files need to be updated, there were 3 but only 2 had to do with 
    the sub-regional selection.
    ...
    if os.path.exists(os.path.join(path, "vdatum")):
        #chesapeak_delaware
        DEdelbay33_file = os.path.join(
            path, "vdatum", "DEdelbay33_8301", "DEdelbay33_8301.kml")
        MDnwchb11_8301_file = os.path.join(
            path, "vdatum", "MDnwchb11_8301", "MDnwchb11_8301.kml")
        MDVAechb1_file = os.path.join(
            path, "vdatum", "MDVAechb11_8301", "MDVAechb11_8301.kml")
        VAswchb11_8301_file = os.path.join(
            path, "vdatum", "VAswchb11_8301", "VAswchb11_8301.kml")
        NJVAmab33_file = os.path.join(
            path, "vdatum", "NJVAmab33_8301", "NJVAmab33_8301.kml")
        NJscstemb32_file = os.path.join(
            path, "vdatum", "NJscstemb32_8301", "NJscstemb32_8301.kml")

        #prvi
        PRVIis_file = os.path.join(
            path, "vdatum", "PRVIis00_8301", "PRVIis00_8301.kml")

        PRVIof_file = os.path.join(
            path, "vdatum", "PRVIof00_8301", "PRVIof00_8301.kml")

        #seak
        AKyakutat00_file = os.path.join(
            path, "vdatum", "AKyakutat00_8301", "AKyakutat00_8301.kml")
        AKglacier00_file = os.path.join(
            path, "vdatum", "AKglacier00_8301", "AKglacier00_8301.kml")
        AKwhale00_file = os.path.join(
            path, "vdatum", "AKwhale00_8301", "AKwhale00_8301.kml")

        #westcoast
        CAsouthn00_file = os.path.join(
            path, "vdatum", "CAsouthn00_8301", "CAsouthn00_8301.kml")
        CAsouin00_file = os.path.join(
            path, "vdatum", "CAsouin00_8301", "CAsouin00_8301.kml")
        CAsfdel00_file = os.path.join(
            path, "vdatum", "CAsfdel00_8301", "CAsfdel00_8301.kml")
        CAsfbay13_file = os.path.join(
            path, "vdatum", "CAsfbay13_8301", "CAsfbay13_8301.kml")
        CAoregon00_file = os.path.join(
            path, "vdatum", "CAoregon00_8301", "CAoregon00_8301.kml")
        CAmontby13_file = os.path.join(
            path, "vdatum", "CAmontby13_8301", "CAmontby13_8301.kml")
        ORcoast00_file = os.path.join(
            path, "vdatum", "ORcoast00_8301", "ORcoast00_8301.kml")
        ORcoain00_file = os.path.join(
            path, "vdatum", "ORcoain00_8301", "ORcoain00_8301.kml")
        WCoffsh00_file = os.path.join(
            path, "vdatum", "WCoffsh00_8301", "WCoffsh00_8301.kml")
        WApuget13_file = os.path.join(
            path, "vdatum", "WApugets13_8301", "WApugets13_8301.kml")
        WAjdfuca14_file = os.path.join(
            path, "vdatum", "WAjdfuca14_8301", "WAjdfuca14_8301.kml")
        WAjdfin00_file = os.path.join(
            path, "vdatum", "WAjdfin00_8301", "WAjdfin00_8301.kml")
        WAcoast00_file = os.path.join(
            path, "vdatum", "WAcoast00_8301", "WAcoast00_8301.kml")
    ...
    #prvi
    PRVIis = read_kml_file(PRVIis_file)
    PRVIof = read_kml_file(PRVIof_file)
    ...
    return {"New Jersey - coastal embayment - South": NJscstemb32,
            ...
            "Puerto Rico and U.S. Virgin Islands - Islands": PRVIis,
            "Puerto Rico and U.S. Virgin Islands - Offshore": PRVIof,
